...
- RC2
- RC4
- [河豚](https://www.enisa.europa.eu/publications/algorithms-key-size-and-parameters-report-2014)
- MD4
- MD5
- SHA1
...
加密算法（例如对称加密或某些 MAC）需要给定大小的秘密输入。例如，AES 使用恰好 16 个字节的密钥。本机实现可能会直接使用用户提供的密码作为输入密钥。使用用户提供的密码作为输入密钥存在以下问题：
...
- [氩2](https://github.com/p-h-c/phc-winner-argon2)
