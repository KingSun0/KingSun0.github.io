...
> 在 OWASP MASTG 中，我们将这个术语称为“身份固定”、“证书固定”、“公钥固定”或简称为“固定”。
...
> 警告：不建议对 Android 应用程序使用证书固定，因为未来服务器配置更改的风险很高，例如更改为另一个证书颁发机构，导致应用程序在未收到客户端软件更新的情况下无法连接到服务器。

它们还包括此[注释](https://developer.android.com/training/articles/security-config#CertificatePinning)：

> 请注意，在使用证书固定时，您应该始终包含一个备份密钥，这样如果您被迫切换到新密钥或更改 CA（当固定到 CA 证书或该 CA 的中间证书时），您的应用程序的连接性不会受到影响。否则，您必须推出应用程序更新才能恢复连接。

第一个声明可能会被错误地解释为他们“不推荐证书固定”。第二个声明澄清了这一点：实际的建议是，如果开发人员想要实施固定，他们必须采取必要的预防措施。
...
- [“Android 安全：SSL 固定”](https://appmattus.medium.com/android-security-ssl-pinning-1db8acb6621e)
- [OWASP 证书固定备忘单](https://cheatsheetseries.owasp.org/cheatsheets/Pinning_Cheat_Sheet.html)
...
- [打嗝非 HTTP 扩展](https://github.com/summitt/Burp-Non-HTTP-Extension)和
...
如果您只是想知道某个敏感数据是否正在传输到网络，则无需部署完全成熟的 MITM 攻击。在这种情况下，如果实施的话，您甚至不必绕过固定。你只需要挂钩正确的功能，例如`SSL_write`从`SSL_read`openssl。
...
- 制作正确的挂钩脚本以重新组装 HTTP 响应对（跨许多方法调用和执行线程）可能非常耗时。您可能会找到[现成的脚本](https://github.com/fanxs-t/Android-SSL_read-write-Hook/blob/master/frida-hook.py)，甚至用于[替代网络堆栈](https://codeshare.frida.re/@owen800q/okhttp3-interceptor/)，但根据应用程序和平台的不同，这些脚本可能需要大量维护并且可能并不*总是有效*。
...
- 在获得 root 权限的设备上，您可以使用挂钩或代码注入来拦截与网络相关的 API 调用（例如 HTTP 请求）并转储甚至操纵这些调用的参数。这消除了检查实际网络数据的需要。我们将在“逆向工程和篡改”章节中更详细地讨论这些技术。
...
- [转储](https://www.tcpdump.org/tcpdump_man.html)
...
### 通过运行时检测设置代理[¶](https://mas.owasp.org/MASTG/General/0x04f-Testing-Network-Communication/#setting-a-proxy-through-runtime-instrumentation)

在获得 root 权限或越狱的设备上，您还可以使用运行时挂钩来设置新的代理或重定向网络流量。这可以通过挂钩工具（如[Inspeckage）](https://github.com/ac-pm/Inspeckage)或代码注入框架（如[Frida](https://www.frida.re/)和[cycript）来实现](http://www.cycript.org/)。您将在本指南的“逆向工程和篡改”章节中找到有关运行时检测的更多信息。
...
- [安卓网络通讯](https://mas.owasp.org/MASTG/Android/0x05g-Testing-Network-Communication/#testing-data-encryption-on-the-network-mstg-network-1)
...
### 安卓[¶](https://mas.owasp.org/MASTG/General/0x04f-Testing-Network-Communication/#android)
